...
export const saveSinglePlayerToDB = async (player: Player) => {
    // TEMPORARILY DISABLED FOR TESTING: if (player.id.startsWith('demo_')) return;
...
export const savePlayersToDB = async (players: Player[]) => {
    // TEMPORARILY DISABLED FOR TESTING: const real = players.filter(p => !p.id.startsWith('demo_'));
    const real = players; 
    if (isSupabaseConfigured()) {
        try {
...
export const saveActiveSessionToDB = async (s: Session | null) => {
    // TEMPORARILY DISABLED FOR TESTING: if (s && !s.id.startsWith('demo_')) await set('activeSession', s);
...
export const saveHistoryToDB = async (history: Session[]) => {
    // TEMPORARILY DISABLED FOR TESTING: const real = history.filter(s => !s.id.startsWith('demo_'));
    const real = history;
...
export const saveNewsToDB = async (news: NewsItem[]) => {
    // TEMPORARILY DISABLED FOR TESTING: const freshNews = news.filter(n => !n.id.startsWith('demo_'));
