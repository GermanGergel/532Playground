...
export const HubProgressChart: React.FC<{ history: PlayerHistoryEntry[], headerTitle?: string }> = ({ history, headerTitle }) => {
    ...
    // ТЕМА ПЕРЕНЕСЕНА ВВЕРХ (До раннего return)
    const theme = useMemo(() => {
        switch (activeMetric) {
    ...
    const areaPath = linePath ? `${linePath} L ${getX(chartData.length - 1)} ${height - paddingBottom} L ${getX(0)} ${height - paddingBottom} Z` : '';
            ...
            <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-start pointer-events-none">
                <div className="flex flex-col gap-3 pointer-events-auto">
                    ...
                    <div className="flex items-center gap-4 pl-1">
                        <div className="flex flex-col justify-center">
                            <span className="text-4xl font-russo text-white leading-none" style={{ color: theme.color, textShadow: `0 0 15px ${theme.color}44` }}>
                                {currentValue}
                            </span>
                        ...
                        <div className="flex flex-col justify-center">
                            <span className={`text-xl font-black leading-none ${isPositiveGrowth ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="flex bg-black/40 rounded-full p-0.5 border border-white/5 pointer-events-auto backdrop-blur-sm">
                    {(['rating', 'winRate', 'goals'] as ChartMetric[]).map(m => (
                        <button key={m} onClick={() => setActiveMetric(m)} className={`px-3 py-1 text-[7px] font-black uppercase rounded-full transition-all ${activeMetric === m ? 'bg-[#00F2FE]/20 text-[#00F2FE] shadow-[0_0_10px_#00F2FE44]' : 'text-white/20 hover:text-white/40'}`}>
                ...
                <div style={{ width: `${width}px`, height: '100%' }}>
                    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
                        ...
                        {[0.2, 0.5, 0.8].map((tick) => {
                            ...
                            return <line key={tick} x1="0" y1={y} x2={width + 2000} y2={y} stroke="white" strokeOpacity="0.03" strokeWidth="1" strokeDasharray="4 4" />;
                        })}
                        ...
                        {chartData.map((entry, index) => {
                            ...
                            return (
                                <g key={index}>
                                    ...
                                    <text x={x} y={y - 12} fill="white" fontSize="9" fontWeight="black" textAnchor="middle" opacity={isLast ? "1" : "0.6"} style={{ textShadow: '0 2px 4px rgba(0,0,0,0.8)' }}>{val}</text>
                                    <text x={x} y={height - 10} fill="white" fillOpacity="0.4" fontSize="9" textAnchor="middle" fontWeight="bold" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>{entry.date}</text>
                                </g>
                            );
                        })}
                    </svg>
