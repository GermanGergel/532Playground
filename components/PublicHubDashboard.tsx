...
// ... (Иконки и вспомогательные функции без изменений) ...
...
const HubCard: React.FC<{ title: React.ReactNode; icon: React.ReactNode; children: React.ReactNode; className?: string; accent?: string; variant?: 'default' | 'dark' | 'midnight' | 'glass' | 'ocean' | 'elite' | 'standings' | 'obsidian'; bodyClassName?: string; headerExtra?: React.ReactNode; align?: 'left' | 'right'; }> = ({ title, icon, children, className = "", accent = "#00F2FE", variant = 'default', bodyClassName = "", headerExtra, align = 'left' }) => {
...
const TacticalRosters: React.FC<{ teams: Team[], players: Player[], session: any, t: any }> = ({ teams, players, session, t }) => (
    <div className="flex h-full w-full divide-x divide-white/10 bg-black/20">
        {teams.map((team) => {
            ...
            let goals = 0; let assistedGoals = 0;
                        ...
                        {team.playerIds.map((pid, idx) => {
                            ...
                            return (
                                <div key={pid} className="group/unit flex-1 flex items-center bg-white/[0.02] rounded-lg px-3 border border-transparent hover:border-white/10 transition-all min-h-[24px]">
                                    <span className="font-mono text-[8px] text-white/5 w-4 shrink-0">{(idx + 1)}</span>
    ...
    const PodiumSpot = ({ p, rank, height, color, delay }: { p?: TopPlayerStats, rank: number, height: string, color: string, delay: string }) => (
        <div className={`flex flex-col items-center justify-end h-full ${delay} animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both relative`}>
            ...
            <div className="w-full relative overflow-hidden backdrop-blur-md rounded-t-xl flex flex-col items-center justify-center pt-2 pb-1.5 z-10" style={{ height: height, background: `linear-gradient(to bottom, ${color}35, ${color}08, transparent)`, borderTop: `2px solid ${color}` }}>
                {p && (<div className="relative z-10 flex flex-col items-center"><span className="font-russo text-xl text-white leading-none tracking-tighter">{p.score.toFixed(1)}</span><span className="text-[5.5px] font-black text-white/50 uppercase tracking-[0.2em] mt-1">{t.hubImpact}</span></div>)}
            </div>
        </div>
    );
    return (
...
const getImpactScore = (stats: PlayerStats): number => {
    let score = 0; const nonCleanSheetWins = stats.wins - (stats.cleanSheetWins || 0);
    score += nonCleanSheetWins * 2.0; score += (stats.cleanSheetWins || 0) * 2.5; score += stats.draws * 0.5; score += stats.goals * 1.0; score += stats.assists * 1.0; score -= stats.ownGoals * 1.0;
    return score;
};
...
export const PublicHubDashboard: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col animate-in fade-in duration-700 w-full relative p-2 md:p-3 overflow-hidden rounded-[2.5rem]">
            {/* 
                Изменено: Фоновый градиент плашки теперь растянут 
                на 100% высоты без лишних отступов снизу.
            */}
            ...
            <div className="flex-grow grid grid-cols-12 gap-4 min-h-0 items-start relative z-10 h-full">
                <div className="col-span-12 md:col-span-9 flex flex-col gap-4 h-full">
                    <div className="flex-[4] min-h-0 shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-200 flex gap-3">
                        ...
                        <HubCard title={t.hubMatchReport} icon={<Target />} accent="#00F2FE" variant="standings" className="flex-1 h-full" bodyClassName="flex flex-col p-5"><MatchEnvironmentWidget session={session} t={t} /></HubCard>
                    </div>
                ...
                <div className="col-span-12 md:col-span-3 flex flex-col gap-4 h-full">
                    ...
                    <HubCard title={<div className="flex gap-14"><button onClick={() => setActiveRightTab('players')} className={`font-russo text-[10px] uppercase tracking-widest ${activeRightTab === 'players' ? 'text-[#00F2FE]' : 'text-white/20'}`}>{t.hubPlayers}</button><button onClick={() => setActiveRightTab('games')} className={`font-russo text-[10px] uppercase tracking-widest ${activeRightTab === 'games' ? 'text-[#00F2FE]' : 'text-white/20'}`}>{t.hubGames}</button></div>} icon={activeRightTab === 'players' ? <Users /> : <HistoryIcon />} variant="standings" accent="#00F2FE" className="flex-grow min-h-0" bodyClassName="flex flex-col h-full min-h-0">
                        <div className="flex-grow overflow-y-auto custom-hub-scrollbar h-full pb-10">
