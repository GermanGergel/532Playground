...
const HubCard: React.FC<{ 
    ...
    if (variant === 'dark' || variant === 'midnight' || variant === 'obsidian') {
        bgStyleClass = 'bg-black/60 backdrop-blur-xl border-white/10';
        headerStyleClass = 'border-b border-white/5';
...
const SessionPodium = React.memo(({ players, t }: { players: TopPlayerStats[], t: any }) => {
    ...
    const PodiumSpot = ({ p, rank, height, color, delay }: { p?: TopPlayerStats, rank: number, height: string, color: string, delay: string }) => {
        const isTotm = p ? totmPlayerIds.has(p.player.id) : false;
                            ...
                            <div className="relative z-10 h-full flex flex-col justify-between p-1.5">
                                ...
                                {isTotm && (
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none opacity-80">
                                        <MiniSquadBadge size="w-12 h-12 md:w-16 md:h-16" />
                                    </div>
                                )}
...
// FIX: Added and exported PublicHubDashboard component to resolve the import error in ClubIntelligenceDashboard.tsx
export const PublicHubDashboard: React.FC = () => {
    const { history, allPlayers, newsFeed } = useApp();
    const t = useTranslation();
    ...
    const { teamStats, allPlayersStats } = useMemo(() => calculateAllStats(latestSession), [latestSession]);
    ...
    const topThree = useMemo(() => {
        return sortedByImpact.slice(0, 3).map((stat, idx) => ({
            player: stat.player,
            score: getImpactScore(stat),
            rank: (idx + 1) as 1 | 2 | 3
        }));
    }, [sortedByImpact]);

    return (
        <div className="h-full w-full p-4 md:p-6 lg:p-8 overflow-y-auto custom-hub-scrollbar">
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-4 h-fit pb-20">
                
                {/* Podium Section - Top Players */}
                <div className="lg:col-span-8 h-[400px]">
                    <HubCard 
                        title={t.hubSessionLeaders} 
                        icon={<TrophyIcon />} 
                        accent="#FFD700" 
                        variant="elite" 
                        className="h-full"
                    >
                        <SessionPodium players={topThree} t={t} />
                    </HubCard>
                </div>
                ...
                <div className="lg:col-span-4 h-[400px]">
                    <HubCard 
                        ...
                        bodyClassName="p-4"
                    >
                        <MatchEnvironmentWidget session={latestSession} t={t} />
                    </HubCard>
                </div>
                ...
                <div className="lg:col-span-7 h-[450px]">
                    <HubCard 
                        title={t.teamStandings} 
                        icon={<TrophyIcon />} 
                        accent="#00F2FE" 
                        variant="standings" 
                        className="h-full"
                    >
                         <div className="flex-grow overflow-y-auto custom-hub-scrollbar p-2">
                             <table className="w-full text-left border-collapse">
                                 ...
                                 <tbody>
                                     {teamStats.map((stat, idx) => (
                                         <tr key={stat.team.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                                             <td className="py-3 px-2 font-mono text-white/30">{idx + 1}</td>
                                             <td className="py-3 px-2">
                                                 <div className="flex items-center gap-3">
                                                     <SubtleDashboardAvatar team={stat.team} />
                                                     <span className="font-bold text-white uppercase text-xs tracking-wider">{stat.team.name}</span>
                                                 </div>
                                             </td>
                                             <td className="py-3 px-2 text-center font-mono text-white/70">{stat.gamesPlayed}</td>
                                             <td className="py-3 px-2 text-center font-mono text-white/70">{stat.wins}</td>
                                             <td className="py-3 px-2 text-center font-black text-[#00F2FE]">{stat.points}</td>
                                         </tr>
                                     ))}
                                 </tbody>
                             </table>
                ...
                <div className="lg:col-span-5 h-[450px]">
                    ...
                    >
                        <TacticalRosters 
                            teams={latestSession.teams} 
                            players={latestSession.playerPool} 
                ...
                <div className="lg:col-span-12 h-fit pb-10">
                    <HubCard 
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {newsFeed.slice(0, 6).map(item => (
                                <NewsVanguardCard key={item.id} item={item} />
                            ))}
                        </div>
                    </HubCard>
                </div>
