...
const SessionPodium: React.FC<{ players: TopPlayerStats[], t: any }> = ({ players, t }) => {
    ...
    const MiniCard = ({ p }: { p: TopPlayerStats }) => {
        ...
        return (
            <div className={`relative rounded-lg overflow-hidden border border-white/20 shadow-2xl flex flex-col shrink-0 ${sizeClasses}`}>
                ...
                <div className="relative z-10 h-full flex flex-col justify-between p-1.5 md:p-2.5">
                    <div className="flex justify-between items-start w-full">
                        {countryCodeAlpha2 && <img src={`https://flagcdn.com/w40/${countryCodeAlpha2.toLowerCase()}.png`} className="w-3 md:w-5 h-auto rounded-sm opacity-90" alt="" />}
                        <div className="flex flex-col items-end leading-none">
                            <span className="font-russo text-lg md:text-2xl text-[#00F2FE]">{p.player.rating}</span>
                            <span className="text-[5px] md:text-[7px] font-black text-white">OVR</span>
                        </div>
                    </div>
                    <div className="w-full text-center pb-1"><span className="text-white font-russo text-[10px] md:text-[13px] uppercase truncate px-1">{p.player.nickname}</span></div>
                </div>
            </div>
        );
    };
    ...
    const PodiumSpot = ({ p, rank, height, color, delay }: { p?: TopPlayerStats, rank: number, height: string, color: string, delay: string }) => (
        <div className={`flex flex-col items-center justify-end h-full ${delay} animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both`}>
            ...
            <div className="w-full relative overflow-hidden backdrop-blur-md rounded-t-xl flex flex-col items-center justify-center pt-3 pb-2" style={{ height: height, background: `linear-gradient(to bottom, ${color}35, ${color}08, transparent)`, borderTop: `3px solid ${color}` }}>
                {p && <div className="relative z-10 flex flex-col items-center"><span className="font-russo text-xl md:text-3xl text-white leading-none">{p.score.toFixed(1)}</span><span className="text-[6px] md:text-[9px] font-black text-white/40 uppercase tracking-widest mt-1">{t.hubImpact}</span></div>}
            </div>
        </div>
    );
