...
const HubPortraitAvatar: React.FC<{ photo?: string; tierColor: string; isSelected?: boolean }> = ({ photo, tierColor, isSelected }) => (
    <div 
        className={`relative w-[45px] h-[58px] rounded-[0.8rem] overflow-hidden bg-[#0C0E12] transition-all duration-500 ease-out z-10 
        border ${isSelected ? 'border-[color:var(--tier-color)] shadow-[0_0_15px_var(--tier-color)]' : 'border-white/10'}
...
export const HubRoster: React.FC<HubRosterProps> = ({ selectedPlayerId, onSelectPlayer, sortBy, setSortBy, search, setSearch, onStartDuel }) => {
    ...
    useEffect(() => {
        if (!isDuelMode && !selectedPlayerId && confirmedPersonnel.length > 0) {
            onSelectPlayer(confirmedPersonnel[0].id);
        }
    }, [confirmedPersonnel, selectedPlayerId, onSelectPlayer, isDuelMode]);

    const toggleDuelMode = () => {
        setIsDuelMode(!isDuelMode);
        if (!isDuelMode) {
            setDuelSlots([null, null]);
        }
    };

    const handleItemClick = (playerId: string) => {
        if (isDuelMode) {
            setDuelSlots(prev => {
                if (prev[0] === playerId) return [prev[1], null];
    ...
    return (
        <div className="absolute inset-0 flex flex-row animate-in fade-in duration-700 overflow-hidden rounded-[2.5rem]">
            {/* SIDEBAR - LEFT COLUMN */}
            <div className="w-[350px] flex flex-col border-r border-white/5 bg-[#080a0e] relative z-20 shrink-0 shadow-[10px_0_30px_rgba(0,0,0,0.5)]">
                {/* SIDEBAR HEADER */}
                <div className="p-6 pb-2 space-y-3 pt-4">
                    <div className="flex items-center justify-start mb-1 pr-2 pl-44 transition-all duration-300">
                        <button 
                            ...
                            className={`group relative flex flex-col items-center px-6 py-2 transition-all active:scale-95 overflow-hidden rounded-xl border ${isDuelMode ? 'bg-[#00F2FE]/20 border-[#00F2FE] shadow-[0_0_20px_rgba(0,242,254,0.3)]' : 'bg-[#00F2FE]/5 border-[#00F2FE]/40 hover:bg-[#00F2FE]/10 hover:border-[#00F2FE]'}`}
                        >
                             <div className="relative z-10 flex flex-col items-center">
                                <span className={`font-blackops text-[24px] uppercase tracking-[0.1em] transition-colors italic leading-none ${isDuelMode ? 'text-white' : 'text-[#00F2FE]'}`}>DUEL</span>
                                <span className={`text-[6px] font-black uppercase tracking-[0.2em] mt-1 transition-colors ${isDuelMode ? 'text-[#00F2FE] animate-pulse' : 'text-white/40'}`}>
                                    {isDuelMode ? 'EXIT SIMULATION' : 'INITIATE SIMULATION'}
                                </span>
                             </div>
                        </button>
                    </div>
                    ...
                    <div className="flex gap-1.5 pr-2">
                        {(['rating', 'name', 'date'] as SortOption[]).map((opt) => (
                            <button 
                                ...
                                className={`flex-1 py-1.5 text-[8px] font-black uppercase tracking-widest rounded-lg border transition-all ${sortBy === opt ? 'bg-[#00F2FE]/10 border-[#00F2FE]/40 text-[#00F2FE]' : 'bg-transparent border-white/5 text-white/20 hover:text-white/40'}`}
                            >
                                {opt}
                            </button>
                        ))}
                ...
                <div className="flex-grow overflow-y-auto custom-hub-scrollbar p-4 pt-4 space-y-3">
                    {confirmedPersonnel.map((person) => {
                        const isSelected = !isDuelMode && selectedPlayerId === person.id;
                        const isDuelSelected = isDuelMode && (duelSlots[0] === person.id || duelSlots[1] === person.id);
                        ...
                        return (
                            <div 
                                ...
                                className={`group/unit relative flex items-center justify-between h-[72px] w-full rounded-2xl transition-all duration-300 cursor-pointer
                                    ${isSelected || isDuelSelected 
                                        ? 'bg-gradient-to-b from-[#1e242c] to-[#0c0e12] border-white/15 shadow-[0_15px_30px_-5px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.1)]' 
                                        : 'bg-gradient-to-b from-[#14181d] to-[#090b0e] border-white/5 hover:border-white/10 shadow-[0_8px_16px_-4px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.05)]'} 
                            ...
                            >
                                ...
                                <div 
                                    ...
                                    style={{ 
                                        backgroundColor: isSelected || isDuelSelected ? (isDuelSelected ? '#00F2FE' : tierColor) : 'transparent', 
                                        boxShadow: isSelected || isDuelSelected ? `0 0 15px ${isDuelSelected ? '#00F2FE' : tierColor}` : 'none',
                                        opacity: isSelected || isDuelSelected ? 1 : 0,
                                    }}
                                ></div>
                                ...
                                <div className="flex items-center px-4 gap-4 w-full">
                                    <div className="shrink-0 transition-transform duration-300 group-hover/unit:scale-105">
                                        <HubPortraitAvatar photo={person.playerCard} tierColor={tierColor} isSelected={isDuelSelected} />
                                    </div>
                                    <div className="flex flex-col min-w-0 flex-grow">
                                        <span className={`font-chakra font-black text-sm uppercase tracking-wide truncate transition-colors ${isSelected || isDuelSelected ? 'text-white' : 'text-white/60'}`}>
                                            {person.nickname}
                                        </span>
                                    ...
                                    <div className="flex flex-col items-end shrink-0">
                                        <div className="flex items-baseline gap-1">
                                            <span 
                                                className={`font-russo text-2xl transition-all duration-300 ${isSelected || isDuelSelected ? 'scale-110' : 'text-white/30'}`}
                                                style={{ 
                                                    color: isSelected || isDuelSelected ? (isDuelSelected ? '#00F2FE' : tierColor) : undefined,
                                                    textShadow: isSelected || isDuelSelected ? `0 0 15px ${isDuelSelected ? '#00F2FE' : tierColor}88` : 'none'
                                                }}
                                            >
                                                {person.rating}
                                            </span>
            ...
            <div className="flex-grow relative bg-[#01040a] overflow-hidden">
                ...
                {isDuelMode ? (
                    <div className="h-full w-full relative animate-in slide-in-from-right-10 duration-700">
                         <HubDuel 
                            p1Id={duelSlots[0]} 
                            p2Id={duelSlots[1]} 
