    ...
    return (
        <div className="absolute inset-0 flex flex-row animate-in fade-in duration-700 overflow-hidden rounded-[2.5rem]">
            ...
            <div className="w-[350px] flex flex-col border-r border-white/5 bg-black/40 relative z-20 shrink-0">
                {/* Header Section: Title shifted right (pl-48), subtitle centered under it */}
                <div className="pt-8 pl-48 flex flex-col items-start shrink-0"> 
                ...
                {/* Search Bar: Aligned with cards (px-4), vertical spacing preserved */}
                <div className="mt-6 px-4 mb-6 shrink-0">
                ...
                {/* Session List: Increased spacing (space-y-4) and card height (h-[80px]) for aesthetic alignment */}
                <div className="flex-grow overflow-y-auto custom-hub-scrollbar p-4 pt-1 space-y-4">
                    {filteredHistory.map((session) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`group relative flex items-center h-[80px] w-full rounded-2xl transition-all duration-300 cursor-pointer border
                                    ${isSelected 
                                        ? 'bg-gradient-to-br from-[#1e2329] to-[#12161b] border-white/20 shadow-[0_15px_30px_rgba(0,0,0,0.7),inset_0_1px_1px_rgba(255,255,255,0.1)]' 
                            ...
                            >
                                {/* Selection indicator: Thin neon strip */}
                                <div 
                                    className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-8 rounded-r-full transition-all duration-500" 
                                    style={{ 
                                        backgroundColor: isSelected ? '#00F2FE' : 'transparent', 
                                        boxShadow: isSelected ? '0 0 15px #00F2FE' : 'none',
                                ...
                                <div className="flex items-center px-4 gap-3 w-full relative z-10">
                                    ...
                                    <div className="flex flex-col min-w-0 flex-grow">
                                        <span className={`font-chakra font-black text-base uppercase tracking-tight truncate transition-colors ${isSelected ? 'text-white' : 'text-white/60'}`}>
                                    ...
                                    <div className="flex gap-3 items-center shrink-0">
                                        <div className="flex flex-col items-end">
                                            <span className={`font-russo text-lg leading-none transition-colors ${isSelected ? 'text-[#00F2FE]' : 'text-white/20'}`}>{session.numTeams || session.teams.length}</span>
                                            <span className="text-[5px] font-mono font-black text-white/10 uppercase">TEAMS</span>
                                        </div>
                                        <div className="w-px h-6 bg-white/5"></div>
                                        <div className="flex flex-col items-end">
                                            <span className={`font-russo text-lg leading-none transition-colors ${isSelected ? 'text-[#00F2FE]' : 'text-white/20'}`}>{session.playerPool.length}</span>
