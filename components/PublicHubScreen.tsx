...
export const PublicHubScreen: React.FC = () => {
    ...
    // Определяем цвет фона в зависимости от текущего вида дашборда для убирания границ
    const getDashboardBg = () => {
        if (!isDashboardOpen) return 'bg-[#0a0c10]';
        switch (dashboardView) {
            case 'dashboard':
            case 'roster':
            case 'duel':
                return 'bg-[#05070a]'; // Цвет плашек Player Hub и основного Дашборда
            case 'archive':
                return 'bg-[#01040a]'; // Цвет плашек меню History (Архив)
            case 'info':
                return 'bg-[#0a0c10]'; // Цвет плашек Information
            default:
                return 'bg-[#0a0c10]';
        }
    };
