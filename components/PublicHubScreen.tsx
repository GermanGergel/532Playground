...
const MotivationalTicker: React.FC = () => {
    const phrases = [
        "DATA BUILDS LEGENDS •",
        "NUMBERS NEVER LIE •",
        "UNIT — GLOBAL FOOTBALL CHANGE •",
        "PERFORMANCE OVER HYPE •",
        "UNIT — THE NEW STANDARD OF FUTSAL •",
        "PROVEN, NOT PROMISED •"
    ];
    return (
...
export const CinematicCard: React.FC<{ player: Player; rank: 1 | 2 | 3 }> = ({ player, rank }) => {
    ...
    const shadowColor = rank === 1 ? 'rgba(255, 215, 0, 0.4)' : rank === 2 ? 'rgba(192, 192, 192, 0.4)' : 'rgba(205, 127, 50, 0.4)';
    const heightClass = rank === 1 ? 'h-80 md:h-96' : rank === 2 ? 'h-72 md:h-80' : 'h-64 md:h-72';
    const zIndex = rank === 1 ? 'z-20' : 'z-10';
    
    return (
        <div className={`relative flex flex-col items-center ${heightClass} w-40 md:w-56 transition-all duration-700 ${zIndex}`}>
            <div 
                className="relative w-full h-full rounded-2xl overflow-hidden border-2 transition-transform hover:scale-105 z-10 bg-[#1A1D24]"
                style={{ 
            ...
            >
                {player.playerCard ? (
                    <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${player.playerCard})` }}></div>
                ) : (
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 flex flex-col items-center">
                    <div className="mb-2 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10">
                        <span className="font-russo text-2xl leading-none" style={{ color: borderColor }}>#{rank}</span>
                    </div>
                    <span className="font-russo text-white text-xl uppercase tracking-tight text-center leading-none mb-1 drop-shadow-md">
                        {player.nickname}
                    </span>
            ...
            {/* Reflection/Shadow */}
            <div className="absolute -bottom-4 w-[90%] h-4 bg-black/50 blur-lg rounded-full"></div>
        </div>
    );
};
    ...
    return (
        <div className="fixed inset-0 w-full h-full bg-[#0a0c10] text-white selection:bg-[#00F2FE] selection:text-black overflow-hidden overscroll-none">
            ...
            <div className={`absolute inset-0 overflow-y-auto overscroll-none touch-pan-y z-10 w-full px-6 md:px-12 transition-all duration-1000 ${isDashboardOpen ? 'opacity-0 scale-95 translate-y-[-100px] pointer-events-none' : 'opacity-100 scale-100 translate-y-0'}`}>
                ...
                {/* ALL-TIME LEGENDS SECTION - MOVED UP */}
                {legends && (
                ...
                {/* CLUB VITALS SECTION - MOVED DOWN */}
                <div className="mt-24 md:mt-32">
                    <div className="text-center mb-12 md:mb-20">
