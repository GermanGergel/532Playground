    ...
    return (
        <div className="flex flex-col h-full py-2 overflow-hidden">
            <div className="flex flex-col gap-4 shrink-0">
                <div className="flex items-center gap-3 border-b border-white/5 pb-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="font-chakra font-bold text-base text-white leading-tight uppercase tracking-wide truncate max-w-[180px] md:max-w-[250px]">
                            {data.location}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="font-mono font-bold text-base text-white tracking-widest leading-none">
                                {data.time}
                            </span>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-900/20 rounded-lg border border-indigo-500/20">
                        <div className="flex flex-col items-end">
                            <span className="font-russo text-xl text-white leading-none">{data.temp}</span>
                            <span className="text-[7px] font-bold text-indigo-300 uppercase tracking-wider">{data.condition}</span>
                        </div>
            ...
            <div className="mt-4 flex-grow flex flex-col min-h-0">
                <div className="flex items-center gap-2 mb-2 opacity-80 shrink-0">
                    <TrophyIcon className="w-3 h-3 text-[#FFD700]" />
                    <span className="text-[9px] font-black text-white/40 uppercase tracking-[0.2em]">SESSION LEADERS</span>
                </div>
                    ...
                    <div className="flex flex-col gap-1.5 bg-black/20 rounded-xl p-2 border border-white/5">
                        {topPlayers.slice(0, 3).map((stat, idx) => ( 
                            <div key={stat.player.id} className="flex items-center justify-between p-1.5 rounded-lg bg-white/[0.03] border border-white/5">
                                <div className="flex items-center gap-3">
                                    <span className="text-sm w-5 text-center">{getMedalIcon(idx)}</span>
                                    <span className={`font-russo text-xs uppercase tracking-wide ${idx === 0 ? 'text-[#FFD700]' : idx === 1 ? 'text-slate-300' : 'text-amber-700'}`}>
                                        {stat.player.nickname || 'Unknown'}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2 pr-2">
                                    <div className="flex flex-col items-end leading-none">
                                        <span className="font-mono font-bold text-sm text-white">{stat.goals + stat.assists}</span>
                                        <span className="text-[6px] text-white/20 uppercase font-bold tracking-wider">PTS</span>
                                    </div>
                                </div>
                            </div>
                        ))}
...
// --- UPDATED HUB CARD TO MATCH BENTO STYLE ---
const HubCard: React.FC<{ 
    ...
    accent?: string;
    isObsidian?: boolean;
}> = ({ title, icon, children, className = "", bodyClassName = "", accent = "#fff", isObsidian = false }) => {
    
    // Using the same style classes as HubPlayerIntel BentoBox
    const cardBase = "relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#161b22] to-[#0a0d14] border border-white/[0.06] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.9),inset_0_1px_1px_rgba(255,255,255,0.05)] group/bento";
    const headerStyleClass = 'bg-transparent border-b border-white/5';
    const iconBg = 'bg-white/10 border-white/20 text-white';

    return (
        <div className={`${cardBase} flex flex-col ${className}`}>
            ...
            <div className={`relative py-3 px-5 flex items-center justify-between shrink-0 ${headerStyleClass} z-10`}>
                 <div className="flex items-center gap-3 relative z-10 w-full">
                    <div className={`w-5 h-5 rounded-md flex items-center justify-center shadow-sm border ${iconBg}`} style={{ color: accent }}>
                        {React.cloneElement(icon as React.ReactElement, { className: "w-3 h-3" })}
                    </div>
                    <div className="flex-grow">
                        {typeof title === 'string' ? ( <h3 className="font-russo text-[11px] uppercase tracking-widest text-white">{title}</h3> ) : ( title )}
                    </div>
                 </div>
            </div>
            <div className={`relative overflow-hidden flex flex-col z-10 ${bodyClassName}`}>
                {children}
            </div>
        </div>
    );
...
export const HubSessionDetail: React.FC<HubSessionDetailProps> = ({ session, onBack }) => {
    ...
    const cleanTeamName = (name: string) => name.replace(/Squad/gi, '').replace(/\s[A-Z0-9]$/i, '').trim();
                ...
                <div className="max-w-6xl mx-auto w-full h-full flex flex-col gap-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 items-start w-full pt-4 h-full min-h-0">
                        ...
                        <div className="flex flex-col gap-5 w-full h-full min-h-0">
                                    ...
                                    <table className="w-full table-fixed border-collapse">
                                        <thead>
                                            <tr>
                                                ...
                                                <th className={`${thClass} w-[45%] text-left pl-4`}>TEAM</th>
                                        ...
                                        <tbody>
                                            {teamStats.map((stat, idx) => (
                                                <tr key={stat.team.id} className="group border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                                                    ...
                                                    <td className={`${tdText} text-left pl-4`}>
                                                        <div className="flex items-center gap-2">
                                                            <TeamAvatar team={stat.team} size="xxs" isLight={true} />
                                                            <div className="flex flex-col leading-none">
                                                                <span className="text-[9px] text-white font-black tracking-widest uppercase mb-0.5 ml-[50px]">SQUAD</span>
                                                                <span className="uppercase text-[10px] font-black tracking-tight truncate group-hover:text-[#00F2FE] transition-colors">
                                                                    {cleanTeamName(stat.team.name || '')}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </td>
                            ...
                            <HubCard title="MATCH REPORT" icon={<Target />} accent="#00F2FE" isObsidian={false} className="w-full flex-grow min-h-0" bodyClassName="p-4 h-full">
                                <ArchiveEnvironmentWidget topPlayers={top3Leaders} session={session} />
                            </HubCard>
                        </div>
