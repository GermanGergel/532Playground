...
const HubNav: React.FC<{ isDashboardOpen: boolean; sessionDate?: string; activeTab: string; onTabChange: (tab: any) => void; archiveViewDate: string | null; onHomeClick: () => void; onOpenTotm: () => void; }> = ({ isDashboardOpen, sessionDate, activeTab, onTabChange, archiveViewDate, onHomeClick, onOpenTotm }) => { const { language, setLanguage } = useApp(); const t = useTranslation(); const [isLangOpen, setIsLangOpen] = useState(false); const dropdownRef = useRef<HTMLDivElement>(null); const languages: { code: Language; label: string }[] = [ { code: 'en', label: 'EN' }, { code: 'ua', label: 'UA' }, { code: 'vn', label: 'VN' }, { code: 'ru', label: 'RU' } ]; useEffect(() => { const handleClickOutside = (event: MouseEvent) => { if (dropdownRef.current && !dropdownRef.current.contains(event.target as Node)) { setIsLangOpen(false); } }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []); const tabTitles: Record<string, string> = { 'dashboard': t.hubDashboardBtn, 'roster': t.playerHub, 'archive': t.navHistory, 'info': t.information }; const navContainerClass = ` fixed top-3 left-1/2 -translate-x-1/2 z-[150] flex items-center justify-between w-full max-w-[1450px] px-6 py-0 bg-black/85 backdrop-blur-xl rounded-2xl border border-white/10 shadow-[0_8px_20px_-6px_rgba(0,0,0,0.5),0_0_15px_rgba(0,242,254,0.1),inset_0_1px_0_rgba(255,255,255,0.05)] h-[42px] md:h-[54px] transition-all duration-300 `; return (<nav className={navContainerClass}><svg className="absolute w-0 h-0 invisible"><filter id="grungeFilter"><feTurbulence type="fractalNoise" baseFrequency="0.25" numOctaves="3" result="noise" /><feDisplacementMap in="SourceGraphic" in2="noise" scale="3" xChannelSelector="R" yChannelSelector="G" /><feComponentTransfer><feFuncA type="linear" slope="0.9" /></feComponentTransfer></filter></svg><div className="flex items-center gap-4 shrink-0 h-full"><div className="flex items-center"><span className="font-blackops text-2xl md:text-3xl text-[#00F2FE] tracking-tighter leading-none" style={{ textShadow: '0 0 10px rgba(0,242,254,0.6)', filter: 'url(#grungeFilter)' }}>UNIT</span><div className="h-4 w-px bg-white/15 ml-3 md:ml-4"></div><div className="flex flex-col space-y-0.5 ml-2"><span className="font-black text-[9px] tracking-[0.15em] text-white uppercase leading-none">Club</span><span className="font-black text-[7px] tracking-[0.15em] text-white/30 uppercase leading-none">Center</span></div><StaticSoccerBall /></div></div><div className="flex-grow h-full flex items-center justify-center pl-[20px] pr-[20px] overflow-hidden min-w-0">{isDashboardOpen ? (<div className="animate-in slide-in-from-bottom-2 fade-in duration-500 flex flex-col items-center justify-center pointer-events-none text-center w-full min-w-0">{activeTab === 'dashboard' ? (<><span className="font-russo text-[7px] text-[#00F2FE] tracking-[0.3em] uppercase leading-none opacity-80 mb-0.5">SESSION BROADCAST</span><span className="font-chakra text-sm md:text-lg font-bold text-white tracking-widest leading-none truncate w-full">{sessionDate || 'LIVE'}</span></>) : (<div className="flex flex-col items-center justify-center w-full">{activeTab === 'archive' && archiveViewDate ? (<div className="flex flex-col items-center"><span className="font-russo text-lg md:text-3xl text-white tracking-tighter uppercase block leading-none truncate w-full" style={{ textShadow: '0 0 25px rgba(255, 255, 255, 0.2)' }}>{archiveViewDate}</span></div>) : (<div className="flex flex-col items-center"><span className="font-russo text-lg md:text-3xl text-white tracking-tighter uppercase block leading-none truncate w-full" style={{ textShadow: '0 0 25px rgba(255, 255, 255, 0.2)' }}>{tabTitles[activeTab] || 'DASHBOARD'}</span></div>)}</div>)}</div>) : (<div className="w-full h-full"><MotivationalTicker /></div>)}</div><div className="flex items-center gap-1 md:gap-3 shrink-0 h-full py-1">{isDashboardOpen && (<div className="flex items-center gap-2 md:gap-4 mr-2 h-full animate-in fade-in slide-in-from-right-3 duration-500"><div className="mr-3 flex items-center border-r border-white/10 pr-4 gap-3"><MiniSquadBadge onClick={onOpenTotm} className="w-[42px] h-[42px] md:w-[54px] md:h-[54px] -my-1 mr-3" /><button onClick={onHomeClick} className="flex flex-col items-center justify-center gap-1 transition-all duration-300 group cursor-pointer hover:scale-110" title="Home"><div className="w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-300 text-white/60 border-white/20 shadow-[0_0_10px_rgba(255,255,255,0.2)] hover:border-white/40 hover:text-white hover:shadow-[0_0_15px_rgba(255,255,255,0.3)]"><Home className="w-4 h-4" /></div><span className="text-[6px] font-black tracking-widest uppercase text-white/30 group-hover:text-white/60 transition-colors">{t.navHome}</span></button><RadioPlayer /></div><NavHubButton title={t.hubDashboardBtn} icon={<LayoutDashboard />} isActive={activeTab === 'dashboard'} onClick={() => onTabChange('dashboard')} /><NavHubButton title={t.playerHub} icon={<Users />} isActive={activeTab === 'roster' || activeTab === 'duel'} onClick={() => onTabChange('roster')} /><NavHubButton title={t.navHistory} icon={<HistoryIcon />} isActive={activeTab === 'archive'} onClick={() => onTabChange('archive')} /><NavHubButton title={t.information} icon={<InfoIcon />} isActive={activeTab === 'info'} onClick={() => onTabChange('info')} /></div>)}<div className="flex items-center gap-2 group h-full relative" ref={dropdownRef}>{!isDashboardOpen && (<div className="hidden md:flex flex-col items-end justify-center h-full animate-in fade-in duration-500 pr-2"><span className="text-[8px] font-black tracking-[0.2em] text-white/30 uppercase group-hover:text-white transition-colors cursor-default leading-none">Language</span></div>)}<div className="relative h-full flex items-center justify-center">{isDashboardOpen ? (<button onClick={() => setIsLangOpen(!isLangOpen)} className={`flex flex-col items-center justify-center gap-1 transition-all duration-300 h-full min-w-[50px] group cursor-pointer hover:scale-110`}><div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-300 ${isLangOpen ? 'text-[#00F2FE]' : 'text-white/60 border-white/10 shadow-[0_0_10px_rgba(255,255,255,0.1)]' }`}><span className="font-black text-[10px] uppercase leading-none">{language}</span></div><span className={`text-[6px] font-black tracking-widest uppercase transition-colors ${isLangOpen ? 'text-[#00F2FE]' : 'text-white/30 group-hover:text-white/60'}`}>LANG</span></button>) : (<button onClick={() => setIsLangOpen(!isLangOpen)} className="w-8 h-8 rounded-full border border-white/20 bg-black/60 flex items-center justify-center transition-all shadow-[0_0_10px_rgba(255,255,255,0.15)] hover:border-white/60 hover:bg-white/5 hover:shadow-[0_0_15px_rgba(255,255,255,0.3)] group/lang"><span className="text-[9px] font-black text-white/80 group-hover/lang:text-white uppercase leading-none transition-colors" style={{ textShadow: '0 0 5px rgba(255,255,255,0.3)' }}>{language}</span></button>)}{isLangOpen && (<div className="absolute left-1/2 -translate-x-1/2 w-9 bg-[#05070a] border border-white/10 rounded-full shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200 z-[250]" style={{ top: isDashboardOpen ? 'calc(100% - 2px)' : '100%', marginTop: isDashboardOpen ? '0' : '8px' }}><div className="py-1 flex flex-col items-center gap-1">{languages.map((lang) => (<button key={lang.code} onClick={() => { setLanguage(lang.code); setIsLangOpen(false); }} className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-300 ${language === lang.code ? 'bg-[#00F2FE]/10 text-[#00F2FE] border border-[#00F2FE]/30 shadow-[0_0_8px_rgba(0,242,254,0.2)]' : 'text-white/40 hover:text-white hover:bg-white/10' }`}><span className="text-[8px] font-black uppercase leading-none">{lang.label}</span></button>))}</div></div>)}</div></div></div></nav>); };
