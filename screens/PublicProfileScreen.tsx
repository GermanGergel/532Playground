...
const MusicLoader: React.FC<{ onInteract: () => void }> = ({ onInteract }) => {
    ...
    return (
        <div 
            className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#1A1D24] cursor-pointer"
            onClick={onInteract}
        >
            <div className="relative flex flex-col items-center justify-center w-64 h-64">
                ...
                <p className="absolute bottom-10 text-sm text-dark-text-secondary animate-pulse">{t.clickToEnter}</p>
            </div>
        </div>
    );
};
